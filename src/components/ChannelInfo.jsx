...
export default function ChannelInfo({ id, name }) {
  ...
  return (
    <Stack direction={'row'} sx={{alignItems: 'center'}} spacing={1} >
      ...
      <Stack spacing={2}>
        <h3>{name}</h3>
        {subscriberCount && <p style={{fontSize:15}}>구독자: {subscriberCount >= 10000 ? `${(subscriberCount / 10000).toFixed(0)}만` : subscriberCount >= 1000 ? `${(subscriberCount / 1000).toFixed(1)}천` : subscriberCount}</p>}
      </Stack>
    </Stack>
  )
}